...
def _initialize_components(auto_instrumentation_version):
    ...
    print("=== 環境変数に基づいた、exporter の生成が行われる...")
    print("🔭🔭🔭 trace_exporters = ", trace_exporters)
    print("🔭🔭🔭 metric_exporters = ", metric_exporters)
    print("🔭🔭🔭 log_exporters = ", log_exporters)        
...
class _OTelSDKConfigurator(_BaseConfigurator):
    ...
    def _configure(self, **kwargs):
        print("😢😢😢 initialize 時は distro は、ここが 実行されるので、この _configure() が実行される... version は ", kwargs.get("auto_instrumentation_version"))
         # version は 0.41b0 が設定        
